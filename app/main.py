...
from . import models
from .auth import verify_password, hash_password, require_permission, require_user, get_current_user
...
def replacement_fetch(request: Request, job_id: str = Form(...), db: Session = Depends(get_db)):
    require_permission(request, 'create_replacement_plate')
    plate_set = db.query(models.PlateSetMaster).filter_by(job_id=job_id.strip()).first()
    if not plate_set:
...
def records_search(request: Request, job_id: str = Form(...), db: Session = Depends(get_db)):
    require_permission(request, 'view_records')
    plate_set = db.query(models.PlateSetMaster).filter_by(job_id=job_id.strip()).first()
    replacements = db.query(models.ReplacementPlateMaster).filter_by(job_id=job_id.strip()).order_by(models.ReplacementPlateMaster.id).all()
    ...
    return templates.TemplateResponse('records.html', context(request, plate_set=plate_set, plate_usage=plate_usage, replacement_rows=replacement_rows, searched=True, searched_job_id=job_id.strip()))
...
def usage_log_create(
    ...
    plate_set = db.query(models.PlateSetMaster).filter_by(plate_set_id=plate_set_id.strip()).first()
    if not plate_set:
        logs = db.query(models.UsageLog).order_by(models.UsageLog.created_at.desc()).limit(50).all()
...
def scrap_plate_set_search(request: Request, search_value: str = Form(...), db: Session = Depends(get_db)):
    ...
    plate_set = db.query(models.PlateSetMaster).filter(
        (models.PlateSetMaster.job_id == search_value.strip()) | (models.PlateSetMaster.plate_set_id == search_value.strip())
    ).first()
...
def scrap_plate_set_request(request: Request, plate_set_id: str = Form(...), reason: str = Form(''), db: Session = Depends(get_db)):
    ...
    plate_set = db.query(models.PlateSetMaster).filter_by(plate_set_id=plate_set_id).first()
    if not plate_set:
    ...
    sr = models.ScrapRequest(request_type='plate_set', target_plate_set_id=plate_set.plate_set_id, target_job_id=plate_set.job_id, requested_by_user_id=user.id, request_reason=reason)
    db.add(sr)
    db.flush()
    owner = db.query(models.User).filter_by(role='owner').first()
...
def scrap_replacement_search(request: Request, plate_set_id: str = Form(...), db: Session = Depends(get_db)):
    require_permission(request, 'raise_scrap_request')
    replacements = db.query(models.ReplacementPlateMaster).filter_by(plate_set_id=plate_set_id.strip()).all()
    return templates.TemplateResponse('scrap_requests.html', context(request, plate_set=None, replacements=replacements, mode='replacement', error=None if replacements else 'No replacement plates found for this Plate Set ID.', success=None, selected_plate_set_id=plate_set_id.strip()))
...
def scrap_replacement_request(request: Request, replacement_plate_id: str = Form(...), reason: str = Form(''), db: Session = Depends(get_db)):
    user = require_permission(request, 'raise_scrap_request')
    replacement = db.query(models.ReplacementPlateMaster).filter_by(replacement_plate_id=replacement_plate_id).first()
    if not replacement:
    ...
    db.add(models.ScrapRequest(request_type='replacement_plate', target_plate_set_id=replacement.plate_set_id, target_replacement_plate_id=replacement.replacement_plate_id, target_job_id=replacement.job_id, requested_by_user_id=user.id, request_reason=reason))
    owner = db.query(models.User).filter_by(role='owner').first()
...
def database_control(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse('database_control.html', context(request, users=users, plate_sets=plate_sets, replacements=replacements))
